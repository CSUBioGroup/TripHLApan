...
class MyDataSet_distribute(tud.Dataset):
    def __init__(self, peps, alleles):
        ...
        encoded_train_peps1 = encode_seq_list(peps, ENCODING_TYPE_PEP1)
        encoded_train_alleles1 = encode_seq_list(alleles, ENCODING_TYPE_ALLELE1)
        self.encoded_peps1 = torch.Tensor(encoded_train_peps1).float()
        self.encoded_alleles1 = torch.Tensor(encoded_train_alleles1).float()

        encoded_train_peps2 = encode_seq_list(peps, ENCODING_TYPE_PEP2)
        encoded_train_alleles2 = encode_seq_list(alleles, ENCODING_TYPE_ALLELE2)
        self.encoded_peps2 = torch.Tensor(encoded_train_peps2).long()
        self.encoded_alleles2 = torch.Tensor(encoded_train_alleles2).long()

        encoded_train_peps3 = encode_seq_list(peps, ENCODING_TYPE_PEP3)
        encoded_train_alleles3 = encode_seq_list(alleles, ENCODING_TYPE_ALLELE3)
        self.encoded_peps3 = torch.Tensor(encoded_train_peps3).float()
        self.encoded_alleles3 = torch.Tensor(encoded_train_alleles3).float()

    def __len__(self):
        return len(self.encoded_peps1)

    def __getitem__(self, index):
        return self.encoded_peps1[index], self.encoded_alleles1[index], self.encoded_peps2[index], self.encoded_alleles2[index], self.encoded_peps3[index], self.encoded_alleles3[index]
...
def test_independent_only_return_list_triple(model_test, independent_dataloader, fold, best_model_name, USE_CUDA, threshold):
    ...
    for i, (X1, X2, X3, X4, X5, X6) in enumerate(independent_dataloader):
        if USE_CUDA:
            X1 = X1.cuda()
            X2 = X2.cuda()
            X3 = X3.cuda()
...
# 注意如果传入的seq不能是连续的str，因为编码时候有两位的数字，反编码会将这种情况割裂，出现bug
